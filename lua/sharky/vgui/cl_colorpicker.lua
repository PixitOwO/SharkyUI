...
function PANEL:UpdateColor()
    self.Color = Sharky.HSLToColor(self.Hue, self.Saturation, self.Lightness)
...
function PANEL:GetHueColor()
    return Sharky.HSLToColor(self.Hue, 1, 0.5)
end

function PANEL:GetColor()
    return Sharky.HSLToColor(self.Hue, self.Saturation, self.Lightness)
end
...
function PANEL:Paint(w, h)
    ...
    surface.SetMaterial(Sharky.ColorPickerGradient)
    surface.DrawPoly(vertices)
    surface.DrawPoly(vertices)
    ...
    surface.SetDrawColor(255, 255, 255, 255)
    surface.SetMaterial(Sharky.ColorPickerWheel)
    surface.DrawTexturedRect(cX - radius, cY - radius, radius * 2, radius * 2)

    local pickerSize = Sharky.resFormat(5)
    local pickerVerts = {
        {
        ...
        {
            x = cX + self.TriX * triangleRadius - Sharky.resFormat(5),
            y = cY + self.TriY * triangleRadius + Sharky.resFormat(5),
            u = 0,
            v = 1
        },
        {
    ...
    surface.SetDrawColor(255, 255, 255, 255)
    surface.SetMaterial(Sharky.ColorPickerPicker)
    ...
    local size = Sharky.resFormat(16)
...
derma.DefineControl('Sharky.ColorPicker', 'Custom themed color picker, should be themed around Pixits style.', PANEL, 'Panel')