...
function PANEL:Init()
	...
	self.btnLeft = vgui.Create('Sharky.Button', self)
	self.btnLeft.PaintOver = function(panel, w, h)
		draw.SimpleText('<', 'Sharky.24', w * 0.5, h * 0.5 - 2, Sharky.TextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end

	self.btnRight = vgui.Create('Sharky.Button', self)
...
function PANEL:PerformLayout()
	...
	self.btnLeft:SetSize(Sharky.resFormat(40), Sharky.resFormat(15))
	self.btnLeft:AlignLeft(Sharky.resFormat(10))
	self.btnLeft:AlignBottom(0)

	self.btnRight:SetSize(Sharky.resFormat(40), Sharky.resFormat(15))
	self.btnRight:AlignRight(Sharky.resFormat(10))
	self.btnRight:AlignBottom(0)
...
derma.DefineControl('Sharky.HorizontalScroller', '', PANEL, 'Panel')